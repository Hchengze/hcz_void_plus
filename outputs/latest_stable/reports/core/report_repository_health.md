# repository health 报告

- 当前 HEAD commit：`e88e5c1`
- latest_stable summary commit：`e88e5c1`
- latest_stable 是否为 Stage 5D：`True`
- figures 根目录 PNG 数量：`0`
- reports 根目录 MD 数量：`0`
- 状态：`pass`

## figures 分层数量

- `core`：`5`
- `forward`：`18`
- `localization`：`3`
- `uncertainty`：`3`
- `diagnostics`：`9`

## reports 分层数量

- `core`：`4`
- `forward`：`6`
- `localization`：`2`
- `uncertainty`：`0`
- `diagnostics`：`4`

## text health 关键统计

| file | lines | cr_only | longest | healthy |
|---|---:|---:|---:|---|
| `README.md` | 159 | 0 | 215 | True |
| `main.py` | 1015 | 0 | 203 | True |
| `docs/forward_modeling_roadmap.md` | 59 | 0 | 278 | True |
| `docs/forward_modeling_boundary.md` | 31 | 0 | 150 | True |
| `docs/elastic2d_forward_design.md` | 47 | 0 | 271 | True |
| `src/forward/acoustic2d/acoustic_fdtd.py` | 88 | 0 | 107 | True |
| `src/forward/forward_registry.py` | 102 | 0 | 107 | True |
| `code/current_3d_algorithm/stable_api.py` | 65 | 0 | 108 | True |
| `outputs/latest_stable/summary.md` | 189 | 0 | 362 | True |
| `outputs/latest_stable/reports/core/report_confidence.md` | 64 | 0 | 362 | True |
| `outputs/latest_stable/reports/core/report_figure_self_check.md` | 56 | 0 | 66 | True |
| `outputs/latest_stable/reports/core/report_full_pipeline.md` | 73 | 0 | 362 | True |
| `outputs/latest_stable/reports/core/report_repository_health.md` | 58 | 0 | 108 | True |
| `outputs/latest_stable/reports/diagnostics/report_model_mismatch.md` | 19 | 0 | 164 | True |
| `outputs/latest_stable/reports/diagnostics/report_velocity_model_ablation.md` | 20 | 0 | 119 | True |
| `outputs/latest_stable/reports/diagnostics/report_velocity_model_audit.md` | 32 | 0 | 109 | True |
| `outputs/latest_stable/reports/diagnostics/report_velocity_model_visualization.md` | 17 | 0 | 97 | True |
| `outputs/latest_stable/reports/forward/report_acoustic2d_prototype.md` | 27 | 0 | 94 | True |
| `outputs/latest_stable/reports/forward/report_elastic2d_das_response.md` | 16 | 0 | 83 | True |
| `outputs/latest_stable/reports/forward/report_elastic2d_rayleigh_validation.md` | 20 | 0 | 89 | True |
| `outputs/latest_stable/reports/forward/report_elastic2d_void_scattering.md` | 17 | 0 | 76 | True |
| `outputs/latest_stable/reports/forward/report_elastic_vs_kinematic.md` | 18 | 0 | 115 | True |
| `outputs/latest_stable/reports/forward/report_forward_engine_ablation.md` | 33 | 0 | 111 | True |
| `outputs/latest_stable/reports/localization/report_geometry_ablation.md` | 16 | 0 | 115 | True |
| `outputs/latest_stable/reports/localization/report_multi_attribute_ablation.md` | 16 | 0 | 104 | True |

- CR-only 文件：`[]`
- 真实一行化/极少行文件：`[]`

GitHub 网页 raw/preview 可能受编码或浏览器渲染影响；本报告以 git ls-tree、本地字节换行和 Python UTF-8 读取结果为准。