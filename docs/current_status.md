...
1. Stage 5D 已完成并推送，当前进入 Stage 5E。
...
15. Stage 5D 的 Rayleigh-like 检测未通过，DAS-like gauge strain 仍很弱，elastic residual 中运动学曲线解释比例很低。
16. Stage 5E 优先加固 elastic2d 数值格式、自由表面、边界、source 和 picking，并澄清 Rayleigh equivalent velocity 与 elastic Vp/Vs/rho 的关系。
17. 未通过 Rayleigh/free-surface 基础验证前，不建议进入 2.5D 或局部 3D elastic。
18. 真实道路场景中，正演真实性、速度模型不确定性、观测几何和 y-depth 耦合必须一起讨论。
...
- 下一步物理正演：`elastic2d_prototype` 精度和稳定性硬化；暂不建议进入 2.5D
- 默认速度模型：`layered`
...
- Stage 5E 科学图件自检：报告/summary 不得把未通过 Rayleigh 或无效 gauge 写成成功
...
- `outputs/latest_stable/figures/forward/fig_elastic2d_numerical_sensitivity_summary.png`
- `outputs/latest_stable/figures/forward/fig_elastic2d_das_response_nonzero_check.png`
- `outputs/latest_stable/figures/diagnostics/fig_velocity_model_physics_bridge.png`
...
- `outputs/latest_stable/reports/core/report_scientific_figure_self_check.md`
