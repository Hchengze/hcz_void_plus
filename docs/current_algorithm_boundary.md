...
Stage 5J 不进入 2.5D，也不继续堆图，而是优先补强三维运动学正演生产代码：x-y-depth 体响应 proxy、速度模型路径积分炮集上下文、经验 Q attenuation 和 forward-localization link。`layered_kinematic` 仍是当前主定位 forward，`elastic2d/staggered` 仍只作 validation。
