...
def build_repository_health_report(repo_root: Path, latest_stable_dir: Path) -> dict[str, Any]:
    ...
    categories = ["core", "forward", "localization", "uncertainty", "diagnostics"]
    figure_counts = {
    ...
    return {
        ...
        "latest_stable_stage5f": "Stage 5F" in (latest / "summary.md").read_text(encoding="utf-8")
...
def write_repository_health_report(path: Path, result: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- latest_stable 是否为 Stage 5F：`{result['latest_stable_stage5f']}`",
