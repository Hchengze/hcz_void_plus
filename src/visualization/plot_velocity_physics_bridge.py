...
def plot_rayleigh_equivalent_vs_elastic_velocity(result: dict[str, Any], output_path: Path) -> None:
    ...
    ax.plot(x, vr, marker="o", label="layered Rayleigh equivalent")
    ax.plot(x, implied_vs, marker="s", label="implied Vs = Vr / 0.9")
    ax.axhline(result["elastic2d_vs_mps"], color="#e15759", linestyle="--", label="elastic2d Vs")
    ax.axhline(
        result["elastic2d_rayleigh_equivalent_mps"],
        color="#f28e2b",
        linestyle=":",
        label="elastic2d 0.9Vs",
    )
    ax.set_xlabel("layer index")
    ax.set_ylabel("velocity / (m/s)")
    ax.set_title("Rayleigh equivalent velocity vs elastic2d Vs")
    ax.legend()
    ax.grid(alpha=0.25)
...
def plot_elastic_vp_vs_rho_model(result: dict[str, Any], output_path: Path) -> None:
    ...
    setup_chinese_matplotlib()
    labels = ["Vp m/s", "Vs m/s", "rho kg/m3", "0.9Vs m/s"]
    values = [
...
def plot_velocity_model_physics_bridge(result: dict[str, Any], output_path: Path) -> None:
    ...
    text = (
        "Velocity Physics Bridge\n"
        f"status: {status}\n"
        f"Vr median = {result['representative_rayleigh_mps']:.1f} m/s\n"
        f"implied Vs = {result['representative_implied_vs_mps']:.1f} m/s\n"
        f"elastic2d Vs = {result['elastic2d_vs_mps']:.1f} m/s\n"
        f"Vs ratio = {result['elastic_vs_to_implied_vs_ratio']:.3f}"
    )
    ax.text(0.5, 0.55, text, ha="center", va="center", fontsize=14, fontweight="bold", color=color)
    ax.text(
        0.5,
        0.12,
        "layered_kinematic: Rayleigh equivalent velocity\nelastic2d: Vp/Vs/rho validation model",
        ha="center",
        va="center",
        fontsize=10,
    )
