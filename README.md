...
当前推进到 **Stage 5I：三维运动学正演-定位一致性修复 + 三维多属性反演增强 + 三维不确定性体输出**。
...
- Stage 5I `staggered_elastic2d_benchmark`：继续作为 validation forward；本轮主线回到三维 `source_xyz / receiver_xyz / candidate_xyz / score_volume(x,y,depth)`。
...
`outputs/latest_stable/` 是当前精选成果目录，不是历史输出仓库。Stage 5I 后，进入该目录的图件必须服务三维算法主线：
